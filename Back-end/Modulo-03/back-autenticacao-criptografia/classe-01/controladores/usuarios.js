...
const cadastrarUsuario = async (req, res) => {
  ...
  try {
    ...
    if (usuario.rowCount > 0) {
      return res.status(400).json("Este email já foi cadastro.");
    }
  } catch (error) {
  ...
  try {
    ...
    const query =
      "insert into usuarios (nome, email, senhaHash) values ($1, $2, $3)";
...
const login = async (req, res) => {
  ...
  try {
    ...
    const result = await pwd.verify(
      Buffer.from(senha),
      Buffer.from(usuario.senhahash, "hex")
    );
    ...
    switch (result) {
      ...
      case securePassword.VALID_NEEDS_REHASH:
        try {
          const hash = (await pwd.hash(Buffer.from(senha))).toString("hex");
          const query = "update usuarios set senhaHash = $1 where email = $2";
